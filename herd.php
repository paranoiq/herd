...
// nope (cloud): BigQuery, Firestore, Athena (Amazon), Redshift (Amazon)
...
$arguments = [
    ...
    'mysql' =>          ['M', Cf::FLAG, $ch("MySQL", "00758f") . "       - OLTP SQL database"],
    'maria' =>          ['A', Cf::FLAG, $ch("MariaDB", "1f305f") . "     - OLTP SQL database, MySQL fork"],
    'postgre' =>        ['P', Cf::FLAG, $ch("PostgreSQL", "336791") . "  - OLTP SQL database"],
    'firebird' =>       ['F', Cf::FLAG, $ch("Firebird", "f41b0a") . "    - OLTP SQL database"],
    'cockroach' =>      ['C', Cf::FLAG, $ch("CockroachDB", "6933ff") . " - OLTP SQL distributed database"],

    'presto' =>         ['',  Cf::FLAG, $ch("Presto", "f41b0a") . "      - OLAP SQL distributed database"],
    'trino' =>          ['',  Cf::FLAG, $ch("Trino", "dd00a1") . "       - OLAP SQL distributed database, Presto fork"],
    // Snowflake S
    // Druid ?

    'mongo' =>          ['G', Cf::FLAG, $ch("MongoDB", "00ed64") . "     - document database"],
    'dynamo' =>         ['',  Cf::FLAG, $ch("DynamoDB", "1c5e9d") . "    - document database, Amazon"],
    'cassandra' =>      ['',  Cf::FLAG, $ch("Cassandra", "1287b1") . "   - column database, Apache"],
    // FerretDB ? (MongoDB-like API on Postgre backend, needs docker-compose)

    'redis' =>          ['R', Cf::FLAG, $ch("Redis", "ff4438") . "       - key-value storage"],
    'valkey' =>         ['V', Cf::FLAG, $ch("Valkey", "6983ff") . "      - key-value storage, Redis fork"],
    'memcached' =>      ['',  Cf::FLAG, $ch("Memcached", "298d83") . "   - key-value storage"],
    'dragonfly' =>      ['',  Cf::FLAG, $ch("Dragonfly", "9143e0") . "   - key-value storage"],
